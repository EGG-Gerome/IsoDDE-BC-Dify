# 20260309-04-V2.0-基于乳腺癌的 IsoDDE API MVP
...
| 模块一原方案问题                                             | 修正方案                     | 影响                                                         |
| ------------------------------------------------------------ | ---------------------------- | ------------------------------------------------------------ |
| SNPeff 是变异注释工具，非序列生成标准工具                    | Biopython 脚本直接替换氨基酸 | 序列准确性提升，可控性增强                                   |
| 只下载 Canonical 序列，UniProt 同源异构体（Isoform）缺失，忽略转录本匹配 | 增加转录本校验说明           | 避免突变位点序列错位                                         |
| 仅支持 TCGA 的 MAF 格式                                      | 预留患者 VCF/PDF 解析接口    | 患者的检测报告通常是 Panel-NGS（几百个基因）或 WES，格式为医院特定的 VCF/PDF |

| 模块二原方案问题                                   | 修正方案                                            | 操作类型 | 影响                                             |
| -------------------------------------------------- | --------------------------------------------------- | -------- | ------------------------------------------------ |
| IsoDDE 是预测引擎，非药物数据库，缺少配体库输入    | 增加「乳腺癌获批药物结构库」作为 IsoDDE 输入        | 增加     | 无配体库 IsoDDE 无法输出药物名称，修正后流程完整 |
| 未区分抗体药与小分子药的输入格式差异               | 优先聚焦小分子靶向药（MVP 阶段），抗体药后续扩展    | 删除     | 降低 MVP 复杂度，小分子药结构预测更成熟          |
| 输出字段与 IsoDDE 技术报告不符（如 Binding Score） | 替换为 IsoDDE 实际输出字段（Affinity、RMSD、pLDDT） | 替换     | 确保输出字段真实可用，避免后续对接错误           |
| 未利用 IsoDDE「隐蔽口袋识别」核心优势              | 增加「突变前后口袋变化分析」步骤                    | 增加     | 提升科研价值，可发现老药新用机会，适合发文章     |

| 模块三原方案问题                                       | 修正方案                                         | 操作类型 | 影响                                       |
| ------------------------------------------------------ | ------------------------------------------------ | -------- | ------------------------------------------ |
| 「突变发生率」权重逻辑错误（群体数据不适用于个体患者） | 替换为「指南推荐级别」（OncoKB Level/NCCN 指南） | 替换     | 符合医生个体化诊疗决策习惯，提升临床可信度 |
| 「耐药率」具体数值难以获取（公开数据库无百分比）       | 改为定性分级（已知耐药/潜在耐药/敏感）           | 替换     | 数据来源可验证，避免编造数值降低可信度     |
| 评分公式过于线性（IsoDDE 得分×权重平均分）             | 采用「门槛 + 排序」机制（临床合规性优先）        | 替换     | 避免实验性药物因物理分数高被错误推荐       |
| 缺少临床解读报告输出                                   | 增加「临床解读报告」生成模块                     | 增加     | 医生可直接使用，无需二次解读               |
